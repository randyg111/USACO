// Source: https://usaco.guide/general/io

import java.io.*;
import java.util.StringTokenizer;

public class DistressSignal {
	public static void main(String[] args) throws IOException {
		BufferedReader r = new BufferedReader(new InputStreamReader(System.in));
		PrintWriter pw = new PrintWriter(System.out);

		int index = 1;
		int sum = 0;
		String packet1 = r.readLine();
		while(packet1 != null) {
			String packet2 = r.readLine();
			if(compare(packet1, packet2) < 0) {
				sum += index;
			}

			r.readLine();
			packet1 = r.readLine();
			index++;
		}
		pw.println(sum);
		
		pw.close();
	}
	public static int compare(String p1, String p2) {
		if(p2.length() == 0) {
			if(p1.length() == 0) {
				return 0;
			} else {
				return 1;
			}
		} else if(p1.length() == 0) {
			return -1;
		}
		if(p1.charAt(0) == '[' && p2.charAt(0) == '[') {
			int b1 = bracket(p1);
			int b2 = bracket(p2);
			
			int c = compare(p1.substring(1, b1), p2.substring(1, b2));
			if(c != 0) return c;
			else return compare(p1.substring(b1+2), p2.substring(b2+2));
			
		} else if(p1.charAt(0) == '[') {
			int b1 = bracket(p1);
			int i = p2.indexOf(",");
			if(i == -1) i = p2.length();

			int c = compare(p1.substring(1, b1), p2.substring(0, i));
			if(c != 0 || i == p2.length() || b1 == p1.length()-1) return c;
			else return compare(p1.substring(b1+2), p2.substring(i+1));
		} else if(p2.charAt(0) == '[') {
			int b2 = bracket(p2);
			int i = p1.indexOf(",");
			if(i == -1) i = p1.length();

			int c = compare(p1.substring(0, i), p2.substring(1, b2));
			if(c != 0 || i == p1.length() || b2 == p2.length()-1) return c;
			else return compare(p1.substring(i+1), p2.substring(b2+2));
		} else {
			int i1 = p1.indexOf(",");
			int i2 = p2.indexOf(",");
			int in1 = 0, in2 = 0;

			if(i1 == -1 && i2 == -1) {
				in1 = Integer.parseInt(p1);
				in2 = Integer.parseInt(p2);
				return in1 - in2;
			} else if(i1 >= 0 && i2 == -1) {
				in1 = Integer.parseInt(p1.substring(0, i1));
				in2 = Integer.parseInt(p2);
				if(in1 - in2 != 0) return in1 - in2;
				else return 1;
			} else if(i1 == -1 && i2 >= 0) {
				in1 = Integer.parseInt(p1);
				in2 = Integer.parseInt(p2.substring(0, i2));
				if(in1 - in2 != 0) return in1 - in2;
				else return -1;
			} else {
				in1 = Integer.parseInt(p1.substring(0, i1));
				in2 = Integer.parseInt(p2.substring(0, i2));
				if(in1 - in2 != 0) return in1 - in2;
				else return compare(p1.substring(i1+1), p2.substring(i2+1));
			}
		}
	}
	public static int bracket(String p) {
		int depth = 0;
		for(int i = 0; i < p.length(); i++) {
			if(p.charAt(i) == '[') {
				depth++;
			} else if(p.charAt(i) == ']') {
				depth--;
				if(depth == 0) return i;
			}
		}
		return depth;
	}
}
